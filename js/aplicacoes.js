...
{  
    ...
    */
    ...
    mensagem = "Ângulo entre o plano inclinado e a base: ";
      ...
      App.strategiesTela.construtorTexto.executa([
        ...
        "#0fc",
        "Bold 16px Trebuchet MS",
        objCanvas.canvasWidth/2 + objCanvas.canvasWidth/4 + ((BASE/22)) - 20,
        380
      ]);
  ...
  //constantes
    ...
    var μe = Math.tan(angRad); //coeficiente atrito estatico para cálculo da máxima Fatestatica ... modificação Canato
    //var μe = 0.1;
    var p = massa * g;
    ...
    //if(μd >= μe){(BASE_Fat = objCanvas.canvasWidth)/(reduzdimensao/Fat);}else{(BASE_Fat = objCanvas.canvasWidth)/(reduzdimensao/px);}
    BASE_Fat = Math.max((objCanvas.canvasWidth)/(reduzdimensao/Fatd), (objCanvas.canvasWidth)/(reduzdimensao/px)); // modificação Canato
    //BASE_Fat = (objCanvas.canvasWidth)/(reduzdimensao/Fat);
  ...
  var desenhaForcas = function(angRad, pontoA, pontoC, BASE_Px, BASE_Py, BASE_Fat){
    ...
    //desenhaReta(pontoPA[0], pontoPA[1], pontoHA[0], pontoHA[1], "#0fc", 3, "3");
     ...
     desenhaReta(NovoXZero, NovoYZero, pontoHB[0], pontoHB[1], "#fff", 3, "3");
     desenhaReta(pontoOB[0], pontoOB[1], pontoHB[0], pontoHB[1], "#fff", 3, "3");
